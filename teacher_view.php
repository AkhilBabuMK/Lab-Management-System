...
if ($resultLabID === false || $resultLabID->num_rows === 0) {
    ...
    $labID = $rowLabID['LabID'];

    // Fetch the list of students enrolled for the specific lab using StudentLab table
    $sqlStudents = "SELECT DISTINCT u.* FROM Users u
                    INNER JOIN StudentLab sl ON u.UserID = sl.StudentID
                    WHERE u.Role = 'Student' AND sl.LabID = $labID";
        ...
        if ($resultStudents === false) {
            ...
            while ($rowStudent = $resultStudents->fetch_assoc()) {
                ...
                $sqlAnswers = "SELECT a.*, q.QuestionNumber
                               ...
                               AND q.LabID = $labID";  // Include LabID in the condition
